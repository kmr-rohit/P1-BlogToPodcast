...
class BlogToPodcastAgent:
    def __init__(self, 
        ...
        if not all([gemini_api_key, elevenlabs_api_key, firecrawl_api_key]):
            raise ValueError("All API keys must be provided")
    ...
    async def scrape_blog_content(self, url: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error scraping blog content: {str(e)}")
            raise ValueError(f"Failed to scrape blog content: {str(e)}")
    ...
    async def generate_podcast(self, url: str) -> dict:
        ...
        try:
            ...
            raise ValueError(f"Failed to generate podcast: {str(e)}")